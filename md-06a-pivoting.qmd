...
Tidyverse Package is a system of packages used in data science for data manipulation, exploration and visualization.
...
```{r}
print(waste_data_long_mean)
```

1.  Long format is readable but may not be the best for comparison as it lists data in a single column.
...
```{r}
waste_data_wide <- waste_data_long_mean |>
  pivot_wider(
    names_from = waste_category, 
    values_from = mean_percent
  )

print(waste_data_wide)
```

Wide format tables are generally easier readable and give a better overview for comparing different variables side by side and are therefore preferable for reports.
...
ggplot(waste_data_long_mean, aes(x = waste_category, y = mean_percent, fill = income_cat)) + 
...
#https://stackoverflow.com/questions/26183958/ggplot-flip-and-transform-coordinate
